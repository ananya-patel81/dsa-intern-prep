...
public:
    int minOperations(vector<int>& nums, int k) {
        ...
        for(int i = 0;i<n;i++){
            ...
            nodes[i].cost = max(0,max(nums[left_ind],nums[right_ind])+1-nums[i]);
            nodes[i].L = left_ind;
            nodes[i].R = right_ind;
            nodes[i].dead = false;
        ...
        priority_queue<pair<int,int>,vector<pair<int,int>>, greater<>> pq(initial_heap.begin(),initial_heap.end());
